...
/**
 * @file utils.c
 * @brief TO library internals utilities implementations.
 */
...
/**
 * @brief Pre-computed table for CRC16 calculation
 */
...
/**
 * @brief Reflect input data around the center bit.
 * @param[in] in Input data
 * @param[in] bitnum Number of bits used by input data
 *
 * @return Reflected data.
 */
static uint16_t bit_reflect(uint16_t in, int bitnum)
{
	int i, j, out;
	out = 0;
	for (i = 1 << (bitnum - 1), j = 1; i != 0; i >>= 1, j <<= 1) {
		if (in & i)
			out |= j;
	}
	return out;
...
uint16_t TO_crc16_ccitt_29b1(uint16_t crc,
		const uint8_t *data, int len, int reflect)
{
	uint8_t d;
	int i;
	crc = bit_reflect(crc, 16);
	for (i = 0; i < len; i++) {
		if (reflect)
			d = bit_reflect(data[i], 8);
		else
			d = data[i];
		crc = (crc << 8) ^ crc16_tbl[(crc >> 8) ^ d];
	}
	if (reflect)
